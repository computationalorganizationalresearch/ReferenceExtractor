...
def random_text_tokens(n, lm="sshleifer/tiny-gpt2"):
    global _GEN
    if _GEN is None:
        try:
...
def make_example(citations, min_words=40, max_words=110, max_refs=4):
    words = random_text_tokens(random.randint(min_words, max_words))
    labels = ["O"] * len(words)
...
def build_dataset(citations, n=2500):
    from datasets import Dataset
    return Dataset.from_list([make_example(citations) for _ in range(n)])
...
def train(citations_file="citations.txt", base_model="distilbert-base-uncased", out_dir="ref-model"):
    citations, ds = load_citations(citations_file), None
    ds = build_dataset(citations)
